...
def render_vis_network(
    vis_nodes, vis_edges, stats, physics_mode: str, height: int = 680
):
...
def _tok_fuzzy_subset(a: str, b: str, thresh: float = 0.76) -> bool:
    ...
    for tok in shorter:
        best = max(
            (difflib.SequenceMatcher(None, tok, lt).ratio() for lt in longer), default=0
        )
        if best < thresh:
            return False
    return True
        ...
        results.append(
            {
                "canonical": canon,
                "variants": sorted(
                    variants, key=lambda x: (-counts.get(x, 0), -len(x))
                ),
                "confidence": conf,
...
def score_column_vs_field(
    col_key: str, profile: dict, field_name: str, field_def: dict
) -> float:
...
_DX_DRUG_BLACKLIST = {
    "B50": {
        "J01": (20, "Malaria+antibiotics: UCG first-line is ACT not J01 antibiotics"),
...
def run_rules_engine(df: pd.DataFrame) -> tuple[pd.DataFrame, dict]:
    ...
    if drug_col:
        summary["rules_available"] += ["R01", "R02", "R03", "R04", "R07", "R08", "R09"]
    if dx_col:
    ...
    for idx, row in df.iterrows():
        ...
        d_name = (
            str(row[drug_nm]).strip() if drug_nm and pd.notna(row.get(drug_nm)) else ""
        )
        ...
        fac = (
            str(row[fac_col]).strip() if fac_col and pd.notna(row.get(fac_col)) else ""
        )
        ...
        vou = (
            str(row[vou_col]).strip() if vou_col and pd.notna(row.get(vou_col)) else ""
        )
        ...
        if d_code and instr and doc:
            ...
            if "HU" in instr_parts:
                if not any(
                    x in doc_up
                    for x in ("HOSPITAL", "INTERN", "SPECIALIST", "SPEC", "SENIOR")
                ):
                    fire(
                        "R01",
                        35,
        ...
        if price > 50000 and dx:
            atc1_dx_ok = {
                "L": {
                    ...
                    "G",
                    "M",
                },  # antineoplastics need cancer/relevant dx
        ...
        if d_code and not drug_info and d_code.upper().startswith("RHIC"):
            fire("R08", 15, f"Procedure {d_code} not found in RAMA tariff", f"{d_code}")
        ...
        rows_out.append(
            {
                ...
                "_decision": decision,
                "_rules_fired": (
                    "; ".join(f"{r['id']}(+{r['score']})" for r in fired)
                    if fired
                    else "—"
                ),
...
def generate_counter_verification_xlsx(
    ...
    dv_verified = DataValidation(
        ...
        error="Please choose YES, NO or PENDING",
    )
    ...
    for _, row in df.iterrows():
        ...
        if pc_col and pc_col in row.index:
            ...
            pc = str(
                _get_col(row, "voucher_id", "paper_code", "Paper Code", default="")
            )
        ...
        row_vals = [
            pc,
            _safe_date(
                _get_col(row, "Dispensing Date", "dispensing_date", "visit_date")
            ),
    ...
    if data_count > 0:
        ...
        tot_border = Border(
            left=MED_BLUE, right=MED_BLUE, top=MED_GOLD, bottom=MED_BLUE
        )
    ...
    ws2.cell(row=tot_row, column=3, value="TOTAL AMOUNT DEDUCTED").alignment = A_RIGHT
    tot_c = ws2.cell(row=tot_row, column=4, value=f"=SUM(D{data_start}:D{tot_row - 1})")
    ...
    sig_data = [
        ...
        (
            4,
            [
                (3, "Lead, Counter Verification", False, C_TEXT),
...
<div style='margin-bottom:6px'>
  ...
  <div style='font-size:11px;color:#475569;font-family:monospace;
    ...
    st.markdown(
        "<hr style='border-color:#1e2a38;margin:10px 0 14px'>", unsafe_allow_html=True
    )
    ...
    st.markdown(
        "<hr style='border-color:#1e2a38;margin:14px 0'>", unsafe_allow_html=True
    )
  ...
  <div style='color:#334155;font-size:10px'>Upload a file and complete<br>Data Prep to activate</div>
    ...
    if _dl_committed:
        st.markdown(
            """<div style='font-size:11px;font-weight:700;color:#64748b;
        text-transform:uppercase;letter-spacing:.06em;margin-bottom:8px'>
        ⚙️ Quick Settings</div>""",
            unsafe_allow_html=True,
        )
        show_raw = st.checkbox("Show raw column names", value=False)
        top_n = st.slider(
            "Top N for charts", 5, 25, _dl.get("top_n", 15), key="sb_top_n"
        )
    else:
        show_raw = False
        top_n = 15
...
# ── Tabs ──────────────────────────────────────────────────────────────────────
tab_dataprep, tab_summary, tab_records, tab_repeat, tab_network, tab_xfac, tab_cv = (
    st.tabs(
        [
            "🗂️ Data Prep",
            "📊 Summary",
...
with tab_summary:
    if not _dl_committed:
        ...
        c[1].metric(
            "Unique Patients",
            f"{s['unique_patients']:,}" if "unique_patients" in s else "—",
        )
        c[2].metric(
            "Repeat Patients",
            f"{s['repeat_patients']:,}" if "repeat_patients" in s else "—",
        )
        c[3].metric(
            "Rapid Revisits", str(len(rapid)), delta=f"≤{rapid_days} day window"
        )

        c2 = st.columns(4)
        c2[0].metric(
            "Unique Practitioners",
            f"{s['unique_doctors']:,}" if "unique_doctors" in s else "—",
        )
        c2[1].metric("Max Visits / Patient", str(s.get("max_visits", "—")))
        c2[2].metric(
            "Total Cost (RWF)",
            fmt_number(s["total_amount"]) if "total_amount" in s else "—",
        )
        c2[3].metric(
            "Avg Cost / Visit",
            fmt_number(s["avg_amount"]) if "avg_amount" in s else "—",
        )
        ...
        with left:
            if "top_patients" in s:
                ...
                fig = hbar_chart(
                    ...
                    td["visits"].tolist(),
                    colors,
                    "Top Patients (RAMA No.) by Visit Count",
                    "Visits",
                )
                if fig:
...
with tab_records:
    if not _dl_committed:
        ...
        st.markdown(
            f'<div class="sec-head">All Records — {len(df):,} rows</div>',
            unsafe_allow_html=True,
        )
        search = st.text_input(
            "🔍 Filter rows", key="rec_search", placeholder="Type to search any column…"
        )
        display_df = df.copy()
        if not show_raw:
            display_df.columns = [
...
with tab_repeat:
    if not _dl_committed:
        ...
        st.markdown(
            "<hr style='border-color:#1e2a38;margin:8px 0 20px'>",
            unsafe_allow_html=True,
        )
            ...
            if not repeat_groups:
                ...
                if len(set(_visit_counts)) > 1:
                    ...
                    for _p, _l in zip(_patches_rp, _bouts):
                        if _l >= 10:
                            _p.set_facecolor(DANGER)
                        elif _l >= 5:
                            _p.set_facecolor(WARN)
                    _ax_rp.set_xlabel("Visits per Patient")
                    _ax_rp.set_ylabel("Patients")
                    _ax_rp.set_title(
                        "Distribution of Repeat Visit Counts",
                        fontsize=11,
                        fontweight="bold",
                        color=TEXT,
                        pad=10,
                ...
                if _top_rp and _top_rp[0]["visits"] >= 5:
                    ...
                    for _tc, _tg in zip(_tp_cols, _top_rp):
                        ...
                        _tvis = _tg["visits"]
                        _tcol = (
                            DANGER if _tvis >= 10 else WARN if _tvis >= 5 else ACCENT
                        )
                        _tc.markdown(
                            f"""
<div style='background:#111720;border:1px solid #1e2a38;
  ...
  <div style='font-size:9px;color:#475569;font-family:monospace'>{_tid}</div>
                ...
                if _rp_srch:
                    _mask = _grp_df.apply(
                        lambda c: c.astype(str).str.contains(
                            _rp_srch, case=False, na=False
                        )
                    ).any(axis=1)
                    _grp_df = _grp_df[_mask]
                _grp_df = _grp_df[_grp_df["visits"] >= _rp_min_visits].head(
                    _rp_max_show
                )
                ...
                st.caption(
                    f"{len(_grp_df):,} patient group(s) shown · "
                ...
                if "doctor" in _grp_df.columns or "doctor_name" in df.columns:
                    with st.expander(
                        "👨‍⚕️ Prescriber involvement in repeat visits", expanded=False
                    ):
                        ...
                        if _dcol and _id_col_df in df.columns:
                            ...
                            _rp_doc_df = df[
                                df[_id_col_df].astype(str).isin(_repeat_ids)
                            ]
                            if not _rp_doc_df.empty:
                                ...
                                st.dataframe(
                                    _doc_counts, use_container_width=True, height=280
                                )
        ...
        # ────────────────────────────────────────────────────────────────────────
            ...
            if not rapid:
                ...
                _rv4.metric(
                    "Unique Patients", _unique_pts, delta=f"≤{rapid_days}-day window"
                )
                ...
                with _rfil3:
                    _rv_sort = st.selectbox(
                        "Sort by",
                        ["Days apart ↑", "Days apart ↓", "Patient name"],
                        key="rv_sort",
                    )
                ...
                if _rv_srch:
                    _rapid_filtered = [
                        r for r in _rapid_filtered if _rv_srch.lower() in str(r).lower()
                    ]
...
with tab_network:
    if not _dl_committed:
        ...
        st.markdown(
            '<div class="sec-head">🔧 Network Configuration</div>',
            unsafe_allow_html=True,
        )
        ...
        with cfg2:
            b_default = col_idx(
                ["patient_name", "patient_id", "gender", "patient_type"]
            )
  ...
  <b style='color:#e2e8f0'>Practitioner Name ↔ Patient Type</b>
        ...
        if col_a == col_b:
            ...
            if vis_nodes is None:
                st.warning(
                    "No edges found. Try lowering the minimum edge weight or choosing different columns."
                )
            else:
                # Stats row
                mc = st.columns(5)
                mc[0].metric(
                    f"◆ {col_a.replace('_',' ').title()[:14]}", stats["nodes_a"]
                )
                mc[1].metric(
                    f"● {col_b.replace('_',' ').title()[:14]}", stats["nodes_b"]
                )
...
<div style='font-size:11px;color:#64748b;margin:8px 0 6px;font-family:monospace'>
  ...
  <b style='color:#e2e8f0'>Search</b> any node by name
                ...
                render_vis_network(
                    vis_nodes, vis_edges, stats, physics_mode, height=net_height
                )
                ...
                if stats["top_a"] or stats["top_b"]:
                    ...
                    with ta:
                        ...
                        st.dataframe(
                            pd.DataFrame(
                                stats["top_a"], columns=["node", "connections"]
                            ).style.background_gradient(
                                subset=["connections"], cmap="Greens"
                            ),
                            use_container_width=True,
                            height=320,
                        )
                    with tb:
                        ...
                        st.dataframe(
                            pd.DataFrame(
                                stats["top_b"], columns=["node", "connections"]
                            ).style.background_gradient(
                                subset=["connections"], cmap="Blues"
                            ),
                            use_container_width=True,
                            height=320,
                        )
...
<div style='margin-top:10px;font-size:11px;color:#64748b;font-family:monospace'>
  ...
  The more facility files uploaded, the more accurate this detection becomes.
        ...
        st.markdown(
            '<div class="sec-head">📂 Step 1 — Upload Hospital & Clinic Visit Files</div>',
            unsafe_allow_html=True,
        )
        st.markdown(
            """
  ...
  <b style='color:#e2e8f0'>The more facilities you upload, the smaller the unverified group becomes.</b>
        ...
        def _parse_facility(raw_bytes, filename):
            ...
            try:
                if fname.endswith(".csv"):
                    raw = pd.read_csv(
                        io.BytesIO(raw_bytes), encoding="utf-8", on_bad_lines="skip"
                    )
                    ...
                    if chosen_df is None:
                        ...
                        for sn in xl.sheet_names:
                            try:
                                sizes[sn] = xl.parse(sn, header=None).shape[0]
                            except:
                                pass
                ...
                if header_row > 0:
                    chosen_df = pd.read_excel(
                        io.BytesIO(raw_bytes), sheet_name=chosen_name, header=header_row
                    )
                else:
                    if (
                        not isinstance(chosen_df.columns[0], str)
                        or chosen_df.columns[0] == 0
                    ):
                ...
                date_c = _find(
                    [r"^date$", r"visit.*date", r"dispensing.*date", r"service.*date"]
                )
                vou_c = _find(
                    [r"voucher.*id", r"voucher.*ident", r"paper.*code", r"invoice.*no"]
                )
                total_c = _find([r"total.*amount", r"total.*cost", r"^total$"])
                doc_c = _find(
                    [r"practitioner", r"prescrib", r"doctor", r"physician", r"medecin"]
                )
                ...
                chosen_df = chosen_df[
                    pd.to_numeric(chosen_df[no_col], errors="coerce").notna()
                ].copy()

                out = pd.DataFrame()
                out["_rama"] = chosen_df[rama_c].astype(str).str.strip().str.upper()
                out["_name"] = (
                    chosen_df[name_c].fillna("").astype(str).str.strip()
                    if name_c
                    else ""
                )
                out["_date"] = (
                    pd.to_datetime(chosen_df[date_c], errors="coerce")
                    if date_c
                    else pd.NaT
                )
                out["voucher_id"] = (
                    chosen_df[vou_c].astype(str).str.strip() if vou_c else ""
                )
                out["total"] = (
                    pd.to_numeric(chosen_df[total_c], errors="coerce").fillna(0)
                    if total_c
                    else 0
                )
                out["doctor"] = (
                    chosen_df[doc_c].fillna("").astype(str).str.strip() if doc_c else ""
                )
        ...
        if xf_uploads:
            ...
            for uf in xf_uploads:
                ...
                if parsed is not None and len(parsed) > 0:
                    ...
                    st.success(
                        f"✅ **{uf.name}** — {len(parsed):,} visit records loaded "
                        f"(sheet: *{parsed['_sheet'].iloc[0]}*)"
                    )
                else:
                    st.error(f"❌ **{uf.name}** — {err}")
            if new_frames:
                st.session_state["fd_facility"] = pd.concat(
                    new_frames, ignore_index=True
                )

        if "fd_facility" not in st.session_state:
            st.info(
                "👆 Upload at least one hospital or clinic file to run fraud detection."
            )
            st.stop()
        ...
        st.markdown(
            '<div class="sec-head">⚙️ Step 2 — Detection Settings</div>',
            unsafe_allow_html=True,
        )
        ...
        ph_work = df.copy()
        ph_work["_rama"] = (
            ph_work[rma_c].astype(str).str.strip().str.upper() if rma_c else ""
        )
        ph_work["_name"] = (
            ph_work[pnm_c].fillna("").astype(str).str.strip() if pnm_c else ""
        )
        ph_work["_date"] = (
            pd.to_datetime(ph_work[dt_c], errors="coerce") if dt_c else pd.NaT
        )
        ph_work["_vou"] = ph_work[vid_c].astype(str).str.strip() if vid_c else ""
        ph_work["_ins"] = (
            pd.to_numeric(ph_work[ins_c], errors="coerce").fillna(0) if ins_c else 0
        )
        ph_work["_tot"] = (
            pd.to_numeric(ph_work[tot_c], errors="coerce").fillna(0) if tot_c else 0
        )
        ...
        # ── STEP 3: Dashboard ─────────────────────────────────────────────────────
        st.markdown(
            '<div class="sec-head">📊 Step 3 — Fraud Detection Dashboard</div>',
            unsafe_allow_html=True,
        )
...
<div style='background:#030d1a;border:1px solid #1e3a5f;border-radius:8px;
  ...
  <b>not yet uploaded</b>. Upload more facility files to reduce false positives.
        ...
        st.markdown(
            "<hr style='border-color:#1e2a38;margin:4px 0 24px'>",
            unsafe_allow_html=True,
        )
    ...
    <span style='font-size:11px;color:#991b1b;font-family:monospace'>
      Patient's RAMA number not found in ANY uploaded facility file
        ...
        if not no_rec.empty:
            ...
            with t1c1:
                t1_srch = st.text_input(
                    "🔍 Search",
                    placeholder="Name, RAMA, voucher, doctor…",
                    key="t1_srch",
                )
            with t1c2:
                t1_doc = st.selectbox(
            ...
            with t1c3:
                t1_min = st.number_input(
                    "Min insurance (RWF)", 0, value=0, step=5000, key="t1_min"
                )
            ...
            t1_show["Dispensing Date"] = (
                pd.to_datetime(t1_show["Dispensing Date"], errors="coerce")
                .dt.strftime("%d/%m/%Y")
                .fillna("—")
            )
            ...
            with st.expander("📊 Prescriber risk breakdown (Table 1)", expanded=False):
                doc_risk = (
                    no_rec.groupby("ph_doctor")["ph_ins"]
            ...
            t1_buf = io.BytesIO()
            _t1_xl = no_rec.copy()
            _t1_xl["ph_date"] = pd.to_datetime(
                _t1_xl["ph_date"], errors="coerce"
            ).dt.strftime("%d/%m/%Y")
        ...
        st.markdown(
            "<hr style='border-color:#1e2a38;margin:28px 0'>", unsafe_allow_html=True
        )
    ...
    <span style='font-size:11px;color:#92400e;font-family:monospace'>
      Patient exists in a facility file but dispensing date is outside the ±{date_window}-day window
        ...
        if not unlinked.empty:
            ...
            with t2c1:
                t2_srch = st.text_input(
                    "🔍 Search", placeholder="Name, RAMA…", key="t2_srch"
                )
            with t2c2:
                t2_max_gap = st.number_input(
                    "Max days apart to show", 1, 365, 60, key="t2_gap"
                )
            ...
            for dcol in ["Pharmacy Date", "Facility Visit Date"]:
                t2_show[dcol] = (
                    pd.to_datetime(t2_show[dcol], errors="coerce")
                    .dt.strftime("%d/%m/%Y")
                    .fillna("—")
                )
            ...
            t2_buf = io.BytesIO()
            _t2_xl = t2_disp.copy()
            _t2_xl["ph_date"] = pd.to_datetime(
                _t2_xl["ph_date"], errors="coerce"
            ).dt.strftime("%d/%m/%Y")
            _t2_xl["fac_date"] = pd.to_datetime(
                _t2_xl["fac_date"], errors="coerce"
            ).dt.strftime("%d/%m/%Y")
      ...
      Legitimate patient journey confirmed — clinic visit → pharmacy dispensing
        ...
        with st.expander("View verified records (Table 3)", expanded=False):
            ...
            for dcol in ["Pharmacy Date", "Facility Date"]:
                t3_show[dcol] = (
                    pd.to_datetime(t3_show[dcol], errors="coerce")
                    .dt.strftime("%d/%m/%Y")
                    .fillna("—")
                )
            if t3_search:
        ...
        st.markdown(
            '<div class="sec-head">⬇️ Download Full Fraud Detection Report</div>',
            unsafe_allow_html=True,
        )

        if st.button(
            "📊 Generate Full Report (4 sheets)", type="primary", key="fd_gen"
        ):
            ...
            def _make_sheet(wb, title, data_df, hdr_color, row_colors):
                ...
                for ci, col in enumerate(data_df.columns, 1):
                    ...
                    c.border = BDR
                    ws.column_dimensions[_gcl(ci)].width = max(
                        14, min(len(str(col)) + 4, 35)
                    )
            ...
            _make_sheet(
                wb, "1 - No Facility Record", t1_xl, "7F1D1D", ["FFE4E4", "FFFFFF"]
            )
            ...
            _make_sheet(
                wb, "2 - Unlinked Visits", t2_xl, "78350F", ["FEF3C7", "FFFFFF"]
            )
...
with tab_cv:
    if not _dl_committed:
        ...
        st.markdown(
            '<div class="sec-head">📄 Counter-Verification Report Generator</div>',
            unsafe_allow_html=True,
        )
...
<div class="info-banner">
  ...
  official two-sheet counter-verification Excel report automatically.
        ...
        st.markdown(
            '<div class="sec-head">📂 Step 1 — Upload Annotated Voucher File</div>',
            unsafe_allow_html=True,
        )
            ...
            try:
                ...
                if fname.endswith(".csv"):
                    ...
                    def score_sheet(df_):
                        ...
                        for col in df_.columns:
                            if re.search(
                                r"observ|remark|reason|explan", str(col).lower()
                            ):
                                non_blank = df_[col].dropna()
                                non_blank = non_blank[
                                    non_blank.astype(str).str.strip() != ""
                                ]
                                if len(non_blank) > 0:
                                    return True
                        return False
                    ...
                    preferred = next(
                        ((sn, d) for sn, d in sheet_candidates if score_sheet(d)), None
                    )
                ...
                def _normalise_col(c):
                    ...
                    return _re.sub(r"[^a-z0-9]", "_", str(c).lower().strip()).strip("_")
                ...
                detected = {
                    ...
                    "ins_copay": _find_col(
                        [r"insurance_co", r"ins_cop", r"couverture", r"rama_amount"]
                    ),
                    "total_cost": _find_col(
                        [r"total_cost", r"total", r"amount", r"cout"]
                    ),
                    "visit_date": _find_col([r"dispensing", r"visit_date", r"date"]),
                    "doctor": _find_col(
                        [r"practitioner", r"prescriber", r"doctor", r"medecin"]
                    ),
                }
                ...
                if _diff_col and _diff_col in raw_ann.columns:
                    _diff_vals = pd.to_numeric(
                        raw_ann[_diff_col], errors="coerce"
                    ).dropna()
                    _has_real_diffs = (
                        len(_diff_vals) > 0 and _diff_vals.abs().max() > 100
                    )
        ...
        if "ann_df" in st.session_state:
            ...
            st.markdown(
                '<div class="sec-head">🔗 Step 2 — Confirm Column Mapping</div>',
                unsafe_allow_html=True,
            )
            st.markdown(
                """
  ...
  and <b style='color:#e2e8f0'>Observation</b> are required.
            ...
            with cc1:
                sel_pc = st.selectbox(
                    "📄 Paper Code",
                    col_opts,
                    index=_sel_idx(detected["paper_code"]),
                    key="cv_sel_pc",
                )
                sel_obs = st.selectbox(
                    "💬 Observation / Reason",
                    col_opts,
                    index=_sel_idx(detected["observation"]),
                    key="cv_sel_obs",
                )
            with cc2:
                sel_dif = st.selectbox(
                    "💰 Difference (amount deducted)",
                    col_opts,
                    index=_sel_idx(detected["difference"]),
                    key="cv_sel_dif",
                )
                sel_ins = st.selectbox(
                    "🏥 Insurance Co-payment",
                    col_opts,
                    index=_sel_idx(detected["ins_copay"]),
                    key="cv_sel_ins",
                )
            with cc3:
                sel_tot = st.selectbox(
                    "💵 Total Cost",
                    col_opts,
                    index=_sel_idx(detected["total_cost"]),
                    key="cv_sel_tot",
                )
                sel_pat = st.selectbox(
                    "👤 Patient Name",
                    col_opts,
                    index=_sel_idx(detected["patient"]),
                    key="cv_sel_pat",
                )
            ...
            if sel_pc != "(none)" and sel_dif != "(none)":
                ...
                ann_work["_obs"] = (
                    ann_work[sel_obs].fillna("").astype(str).str.strip()
                    if sel_obs != "(none)"
                    else ""
                )
                ann_work["_ins"] = (
                    ann_work[sel_ins].apply(_to_float) if sel_ins != "(none)" else 0.0
                )
                ann_work["_tot"] = (
                    ann_work[sel_tot].apply(_to_float) if sel_tot != "(none)" else 0.0
                )
                ann_work["_pat"] = (
                    ann_work[sel_pat].fillna("").astype(str).str.strip()
                    if sel_pat != "(none)"
                    else ""
                )
                    ...
                    deduction_list.append(
                        {
                            ...
                            "amount": -abs(
                                r["_dif"]
                            ),  # always negative regardless of source sign
                            "explanation": r["_obs"],
                            "ins_copay": r["_ins"],
                            "total_cost": r["_tot"],
                        }
                ...
                st.markdown(
                    '<div class="sec-head">📊 Step 3 — Deduction Summary</div>',
                    unsafe_allow_html=True,
                )
...
<div style='font-size:11px;color:#64748b;font-family:monospace;margin:8px 0 4px'>
  <b style='color:#e2e8f0'>Deduction reasons breakdown:</b>
                ...
                st.info(
                    "👆 Assign at least the **Paper Code** and **Difference** columns above to continue."
                )

            # ── STEP 4 — Report metadata ───────────────────────────────────────
            if deduction_list:
                st.markdown(
                    '<div class="sec-head">📋 Step 4 — Report Metadata</div>',
                    unsafe_allow_html=True,
                )

                mc1, mc2 = st.columns(2)
                with mc1:
                    cv_province = st.text_input(
                        "Province", value="WESTERN PROVINCE", key="cv_prov"
                    )
                    cv_district = st.text_input(
                        "Administrative District", value="RUBAVU", key="cv_dist"
                    )
                    cv_pharmacy = st.text_input(
                        "Pharmacy", value="PHARMACIE VINCA GISENYI LTD", key="cv_pharm"
                    )
                with mc2:
                    if "date_min" in s and "date_max" in s:
                        ...
                        default_period = ""
                    cv_period = st.text_input(
                        "Period", value=default_period, key="cv_per"
                    )
                ...
                with sc1:
                    cv_verified = st.text_input(
                        "Verified by", value="Alphonsine MUKAKAYIBANDA", key="cv_verif"
                    )
                with sc2:
                    cv_approved = st.text_input(
                        "Approved by", value="", key="cv_approv"
                    )

                # ── STEP 5 — Generate ──────────────────────────────────────────
                st.markdown(
                    '<div class="sec-head">⬇️ Step 5 — Generate Report</div>',
                    unsafe_allow_html=True,
                )

                if st.button(
                    "📊 Generate Counter-Verification Excel Report",
                    type="primary",
                    key="cv_gen_btn",
                ):
                    meta = {
                ...
                if st.session_state.get("cv_generated"):
                    pharm_s, period_s = st.session_state.get(
                        "cv_fname_meta", ("pharmacy", "period")
                    )
                    fname = (
                        f"counter_verification_"
    ...
    def _step_bar(active: int):
        ...
        for i, (num, label) in enumerate(steps, 1):
            ...
            sep = (
                ""
                if i == len(steps)
                else "<div style='width:2px;background:#1e2a38;flex-shrink:0'></div>"
            )
            html += f"""
  ...
  <div style='font-size:13px;color:#475569;font-family:monospace'>
    ...
    if _dp_cache_key not in st.session_state:
        with st.spinner(f"Loading {_raw_fname}…"):
            try:
                ...
                if _fn.endswith(".csv"):
                    _dp_df = pd.read_csv(
                        io.BytesIO(_raw_bytes), encoding="utf-8", on_bad_lines="skip"
                    )
                    ...
                    for _sn in _xl.sheet_names:
                        try:
                            ...
                            _scores[_sn] = len(_tmp.columns) * 1000 + len(
                                _xl.parse(_sn, nrows=500)
                            )
                        except Exception:
                            _scores[_sn] = 0
    ...
    _step_bar(
        ...
        if not st.session_state.get("dp_step1_done")
            ...
            else 3 if not st.session_state.get("dp_step3_done") else 4
        )
    )

    with st.expander(
        "📄 Step 1 — File Overview",
        expanded=not st.session_state.get("dp_step1_done", False),
    ):
        ...
        _s1c4.metric(
            "Est. size", f"{_dp_df.memory_usage(deep=True).sum() / 1024:.0f} KB"
        )
        ...
        for _c in _dp_df.columns:
            ...
            _fill_color = (
                "#031a0a"
                if _null_pct < 5
                else "#1a1000" if _null_pct < 30 else "#1a0505"
            )
            _fill_text = (
                "#22c55e"
                if _null_pct < 5
                else "#f59e0b" if _null_pct < 30 else "#ef4444"
            )
            _qual_rows.append(
                {
                    "Column": str(_c)[:40],
        ...
        if st.button(
            "✅ Looks good — proceed to column mapping",
            type="primary",
            key="dp_s1_next",
        ):
    ...
    with st.expander(
        "🗺️ Step 2 — Map Columns to System Fields",
  ...
  <div style='font-size:12px;color:#64748b;font-family:monospace;line-height:1.9'>
    ...
    don't exist in your data.
        ...
        for _c in _src_cols:
            ...
            _samp = " · ".join(str(v) for v in _s_c.dropna().head(3).tolist())[:55]
        ...
        for _grp in _GROUPS_ORDER:
            _fields_in_grp = [
                (f, d) for f, d in _SYSTEM_FIELDS.items() if d["group"] == _grp
            ]
                ...
                with _right:
                    ...
                    if _sel_col != _col_none_opt and _sel_col in _col_meta:
                        ...
                        _fill_c = (
                            "#22c55e"
                            if _m["null_pct"] < 5
                            else "#f59e0b" if _m["null_pct"] < 30 else "#ef4444"
                        )
                        st.markdown(
                            f"""
...
<div style='font-size:11px;color:#475569;font-family:monospace;margin-bottom:12px'>
  ...
  <code style='color:#64748b'>raw_</code> prefix) or exclude it from the data lake entirely.
        ...
        _sb_req_done = (
            ...
            - (
                len(_SYSTEM_FIELDS)
                - sum(1 for d in _SYSTEM_FIELDS.values() if d["required"])
            )
        )
    ...
    with st.expander(
        ...
        for _c in _prev_clean.columns:
            ...
            _status = (
                "✅ OK"
                if _null_p < 5
                else "⚠️ Sparse" if _null_p < 30 else "🔴 High nulls"
            )
            _qual2.append(
                {
                    "Column": _c,
        ...
        st.markdown(
            '<div style="font-size:11px;font-weight:700;color:#64748b;'
            'text-transform:uppercase;letter-spacing:.06em;margin:14px 0 6px">'
            "Prepared Data Preview (first 50 rows)</div>",
            unsafe_allow_html=True,
        )
        ...
        _doc_col_dp = next(
            (
                c
                for c in ["doctor_name", "practitioner_name"]
  ...
  and suggests merges. Approve the ones you want before committing to the lake.
            ...
            if not _clusters:
                ...
                for _ci_dp, _cl in enumerate(_clusters[:40]):
                    ...
                    _conf_c = DANGER if _susp else WARN if _conf < 0.85 else ACCENT
                    _freq_total = sum(_doc_counts.get(v, 0) for v in _vars)
                    _variant_pills = "".join(
                    ...
                    _checked = st.session_state.get(
                        _key_dp, _conf >= 0.88 and not _susp
                    )
                    st.markdown(
                        f"""
<div style='background:#111720;border:1px solid #1e2a38;
    ...
    {_variant_pills}
                ...
                if _approved_dp:
                    ...
                    if st.button(
                        ...
                        _n_merged = sum(len(c["variants"]) for c in _approved_dp)
                        st.success(
...
<div style='font-size:11px;color:#334155;font-family:monospace;
  ...
  Map a <code>doctor_name</code> field in Step 2 to enable name normalisation.
        ...
        st.markdown(
            '<div class="sec-head">🔧 Optional: rename raw_ columns</div>',
            unsafe_allow_html=True,
        )
        ...
        with _s3c1:
            if st.button(
                "✅ Confirm & Proceed to Commit", type="primary", key="dp_s3_next"
            ):
                st.session_state["dp_preview_clean"] = _prev_clean
    ...
    _final_df = st.session_state.get(
        "dp_preview_clean",
        apply_column_mapping(
            _dp_df.drop(columns=list(_confirmed_exc), errors="ignore"), _confirmed_map
        ),
    )
    ...
    with st.expander("🚀 Step 4 — Commit to Data Lake", expanded=True):
        ...
        for _fld, _orig in _confirmed_map.items():
            _gc2 = _GROUP_COLORS.get(
                _SYSTEM_FIELDS.get(_fld, {}).get("group", ""), "#64748b"
            )
            _chip_parts.append(
...
<div style='background:#030d1a;border:1px solid #1e3a5f;border-radius:12px;
  ...
  <div style='font-size:11px;color:#64748b;font-family:monospace;line-height:1.9'>
    ...
    • Clearing the sidebar file upload or clicking <b style='color:#ef4444'>Clear Data Lake</b> resets everything
        ...
        with _reset_col:
            if st.button(
                "↩ Re-do column mapping", use_container_width=True, key="dp_redo"
            ):
                for _k in [
                    "dp_step1_done",
                    "dp_step2_done",
                    "dp_step3_done",
                    "dp_confirmed_mapping",
                    "dp_confirmed_exclude",
                    "dp_preview_clean",
                    "dp_rename_overrides",
                ]:
                    st.session_state.pop(_k, None)
